...
namespace sstmac {
  namespace hw {
    ...
    bool
    packetizer::pm_mark_packet(pending_send& msg_info, long bytes, long byte_offset) {
      if((static_cast<network_message*>(msg_info.msg))->is_pm_monitor()) {
	if ((byte_offset + bytes) == (msg_info.msg)->byte_length()) { //is tail packet?
	  return true;
	} else {
	  return false;
	}
